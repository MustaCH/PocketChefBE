...
import { getSpecificRecipe } from "./ai/flows/get-specific-recipe";
import { generateEventRecipes } from "./ai/flows/generate-event-recipes";
...
app.post("/api/get-specific-recipe", async (req, res) => {
  try {
    const result = await getSpecificRecipe(req.body);
    res.json(result);
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
app.post("/api/generate-event-recipes", async (req, res) => {
  try {
    const result = await generateEventRecipes(req.body);
    res.json(result);
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
